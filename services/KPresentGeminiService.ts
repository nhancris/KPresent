...
const initializeGemini = () => {
  if (typeof process !== 'undefined' && process.env && process.env.API_KEY) {
    ...
    console.warn("API_KEY not found in process.env. KPresentGeminiService will use mock responses.");
    ai = null;
  }
};
...
const generateMockSvg = (title: string, contentText: string, themeDesc: string, layoutHint: SlideLayout, theme: Theme): string => {
  ...
  switch (layoutHint) {
    ...
    case SlideLayout.TITLE_ONLY: // For "Thank You" or similar
      svgElements += `<rect x="0" y="0" width="800" height="450" fill="${bgColor}" />`; // Ensure full background
      ...
      svgElements += wrapText(safeTitle, 400, 200, 700, 1.2, headingFont, 60, titleColor, "bold", "middle");
      if (safeContentText && safeContentText.trim() !== '' && safeContentText.trim().toLowerCase() !== "q&a") {
          svgElements += wrapText(safeContentText, 400, 280, 600, 1.2, bodyFont, 30, textColor, "normal", "middle");
      } else if (safeContentText && safeContentText.trim().toLowerCase() === "q&a") {
          svgElements += wrapText("Q & A", 400, 280, 600, 1.2, bodyFont, 30, textColor, "normal", "middle");
      }
      break;
...
export const generateSlideDataWithGemini = async (
  ...
  const systemInstruction = `You are an AI KPresent Design Agent. Your primary task is to generate data for a single, visually stunning KPresent slide.
...
Overall KPresent Context: "${overallPrompt}"
...
export const refineImagePromptWithGemini = async (originalPrompt: string, generationMode: GenerationMode = GenerationMode.NORMAL): Promise<string> => {
  if (!ai) {
    return `Mock refined (${generationMode}): High-quality, professional image depicting ${originalPrompt}, suitable for a KPresent, clear visuals.`;
  }
