...
import { generateSlideDataWithGemini } from './KPresentGeminiService';
...
export const generateMockKPresent = async (
  prompt: string,
  ...
  const overallTopic = prompt.length > 5 ? prompt.substring(0, Math.min(prompt.length, 60)).split(',')[0].trim() : "AI KPresent";
  ...
  if (numSlidesRequested > 1 && slides.length < numSlidesRequested) { // Ensure conclusion is added if space allows
    ...
    slides.push({
        ...
        elements: [],
        speakerNotes: conclusionData.speakerNotes || "Conclude the KPresent, summarize key takeaways, and open for questions.",
