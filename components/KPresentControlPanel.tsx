...
// import { generateSlideDataWithGemini } from '../services/KPresentGeminiService'; // For re-generation

interface KPresentControlPanelProps {
  kpresent: KPresent;
...
export const KPresentControlPanel: React.FC<KPresentControlPanelProps> = ({
  kpresent,
  activeSlide,
  onSlideUpdate,
  onThemeChange,
  ...
  const renderSlideControls = () => {
    ...
    return (
      <>
        ...
        <ControlInput label="Background Color (Fallback)" id="slideBgColor" type="color" value={activeSlide.backgroundColor || kpresent.theme.colorPalette.slideBackground || '#FFFFFF'} onChange={(e) => onSlideUpdate(activeSlide.id, { backgroundColor: e.target.value })} />
  ...
  const renderThemeControls = () => (
    <>
      <ControlInput label="KPresent Theme" id="globalTheme" value={kpresent.theme.id} onChange={(e) => {
      ...
      <div className="mt-2 p-2 border border-slate-600 rounded">
        ...
        <ControlInput label="Heading Font" id="headingFont" value={kpresent.theme.fontPairing.heading} onChange={(e) => onThemeChange({...kpresent.theme, fontPairing: {...kpresent.theme.fontPairing, heading: e.target.value}})}>
            {DEFAULT_FONTS.map(f => <option key={f.heading} value={f.heading}>{f.heading.split(',')[0].replace(/"/g, '')}</option>)}
        </ControlInput>
        <ControlInput label="Body Font" id="bodyFont" value={kpresent.theme.fontPairing.body} onChange={(e) => onThemeChange({...kpresent.theme, fontPairing: {...kpresent.theme.fontPairing, body: e.target.value}})}>
            {DEFAULT_FONTS.map(f => <option key={f.body} value={f.body}>{f.body.split(',')[0].replace(/"/g, '')}</option>)}
        </ControlInput>
        <ControlInput label="Primary Color" id="primaryColor" type="color" value={kpresent.theme.colorPalette.primary} onChange={(e) => onThemeChange({...kpresent.theme, colorPalette: {...kpresent.theme.colorPalette, primary: e.target.value}})} />
        <ControlInput label="Accent Color" id="accentColor" type="color" value={kpresent.theme.colorPalette.accent} onChange={(e) => onThemeChange({...kpresent.theme, colorPalette: {...kpresent.theme.colorPalette, accent: e.target.value}})} />
         <ControlInput label="Default Slide Background (Fallback)" id="slideDefaultBgColor" type="color" value={kpresent.theme.colorPalette.slideBackground || kpresent.theme.colorPalette.background} onChange={(e) => onThemeChange({...kpresent.theme, colorPalette: {...kpresent.theme.colorPalette, slideBackground: e.target.value}})} />
      </div>
    </>
  );
