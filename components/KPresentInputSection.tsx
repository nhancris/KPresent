...
interface KPresentInputSectionProps {
...
export const KPresentInputSection: React.FC<KPresentInputSectionProps> = ({ onGenerate, initialPrompt = "" }) => {
  ...
  const handleSubmit = useCallback((event: React.FormEvent) => {
    ...
    if (!prompt.trim()) {
      alert("Please enter a topic for your KPresent.");
      return;
    }
  ...
  return (
    <div className="w-full max-w-2xl p-8 bg-slate-800 shadow-2xl rounded-xl">
      <form onSubmit={handleSubmit}>
        <InputField label="What is your KPresent about?" id="prompt">
          <textarea
            id="prompt"
            rows={4}
